...
exports.signUp = async (req, res) => {
  try {
    ...
    //validate credentials or Check if All Details are there or not
    if (
      !firstName ||
      !lastName ||
      !email ||
    ...
    if (password != confirmPassword) {
      return res.status(400).json({
        success: false,
        message: "Password values does not match, Please fill carefully!",
      });
    }
    ...
    if (existingUser) {
      return res.status(400).json({
        success: false,
        message: "User is already registered, Please Log in to continue.",
      });
    }
...
exports.changePassword = async (req, res) => {
  try {
    ...
    const userDetails = await User.findById(user.req.id);
